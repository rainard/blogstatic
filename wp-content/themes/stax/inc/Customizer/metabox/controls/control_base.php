...
abstract class Control_Base {
	...
	public function render( $post_id ) {
		...
		wp_nonce_field( 'stax_meta_box_nonce', 'stax_meta_box_process' );
	}
	...
	final public function save( $post_id ) {
		if ( ! isset( $_POST['stax_meta_box_process'] ) ) {
			return;
		}
		if ( ! wp_verify_nonce( sanitize_key( $_POST['stax_meta_box_process'] ), 'stax_meta_box_nonce' ) ) {
			return;
		}
		...
		if ( isset( $_POST[ $this->id ] ) ) {
			...
			if ( $this->id === 'stax_meta_enable_content_width' ) {
				update_post_meta( $post_id, 'stax_meta_enable_content_width', 'off' );
			} else {
				delete_post_meta( $post_id, $this->id );
			}
			return;
