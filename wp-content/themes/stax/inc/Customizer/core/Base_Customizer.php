...
use Stax\Customizer\Core\Types\Partial;
...
abstract class Base_Customizer {
	...
	private function register_controls() {
		...
		foreach ( $controls as $control ) {
			...
			if ( null !== $control->custom_control ) {
			...
			if ( isset( $control->control_args['live_refresh_selector'] ) && false !== $control->control_args['live_refresh_selector'] ) {
				$control_args = [
			...
			if ( isset( $control->control_args['conditional_header'] ) && $control->control_args['conditional_header'] ) {
				$id = $control->id;
				add_filter(
					'stax_pro_react_controls_localization',
