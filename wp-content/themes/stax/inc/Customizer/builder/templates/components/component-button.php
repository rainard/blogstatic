...
use Stax\Builder\Core\Components\Button;

$text = parse_dynamic_tags( component_setting( Button::TEXT_ID ) );
$text = apply_filters( 'stax_translate_single_string', $text, Button::TEXT_ID );
if ( ! empty( $text ) ) {
	$button_link = parse_dynamic_tags( component_setting( Button::LINK_ID ) );
	$button_link = apply_filters( 'stax_translate_single_string', $button_link, Button::LINK_ID );
