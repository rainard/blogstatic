...
use Stax\Builder\Core\Customizer;
use Stax\Builder\Core\Settings\Manager;
...
class Main {
	 ...
	 */
	const TEMPLATES_DIRECTORY = 'inc/Customizer/builder/templates/';
	/**
	 ...
	 *
	 * @return \Stax\Builder\Main Instance.
	 * @since   1.0.0
	 * @access  public
	 */
	...
	public function init() {
		add_filter( 'stax_style_subscribers', [ $this, 'inline_styles' ] );
	}
	...
	private function should_enqueue_assets() {
		$disabled_templates = [ 'elementor_canvas' ];
		...
		if ( is_singular() ) {
			$id     = get_the_ID();
			$header = get_post_meta( $id, 'stax_meta_disable_header', true );
			$footer = get_post_meta( $id, 'stax_meta_disable_footer', true );
