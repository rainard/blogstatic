...
class Theme {
			...
			if ( ! $component instanceof Component_Interface ) {
				throw new InvalidArgumentException(
					...
					/* translators: 1: classname/type of the variable, 2: interface name */
						__( 'The theme component %1$s does not implement the %2$s interface.', 'stax' ),
						gettype( $component ),
	 ...
	 *
	 * Calling `stax()` is a short-hand for calling this method on the main theme instance. The instance then allows
	...
	public function component( $slug ) {
		if ( ! isset( $this->components[ $slug ] ) ) {
			throw new InvalidArgumentException(
				sprintf(
				/* translators: %s: slug */
					__( 'No theme component with the slug %s exists.', 'stax' ),
					$slug
				)
			);
		}
	...
	protected function get_default_components() {
		$components = [
			...
			new Date\Component(),
			new Post_Navigation\Component(),
			new Elementor\Component(),
			new Load_Template\Component(),
			new Load_Icon\Component(),
