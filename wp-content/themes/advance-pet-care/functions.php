...
function advance_pet_care_widgets_init() {
	register_sidebar(array(
		...
		'after_widget'  => '</aside>',
		'before_title'  => '<h3 class="widget-title text-capitalize text-start p-2">',
		'after_title'   => '</h3>',
	));
...
if ( ! function_exists( 'advance_pet_care_related_posts' ) ) {
	function advance_pet_care_related_posts() {
		wp_reset_postdata();
		global $post;
		$args = array(
			'no_found_rows'          => true,
			...
			'posts_per_page'    => absint( get_theme_mod( 'advance_pet_care_related_posts_number', '3' ) ),
		);
		// Categories
		if ( get_theme_mod( 'advance_pet_care_related_posts_taxanomies_options', 'categories' ) == 'categories' ) {
		...
		// Tags
		if ( get_theme_mod( 'advance_pet_care_related_posts_taxanomies_options', 'categories' ) == 'tags' ) {
