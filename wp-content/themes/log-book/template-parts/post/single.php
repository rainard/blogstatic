                        ...
                        <li class="meta-comment list-inline-item">
                            <?php $cmt_link = get_comments_link(); 
                                  $num_comments = get_comments_number();
                                    if ( $num_comments == 0 ) {
                                        $log_book_comment = __( 'No Comments', 'log-book' );
                                    } elseif ( $num_comments > 1 ) {
                                        $log_book_comment = $num_comments . __( ' Comments', 'log-book' );
                                    } else {
                                        $log_book_comment = __('1 Comment', 'log-book' );
                                    }
                                  
                            ?>  
                            <i class="fa fa-comment-o" aria-hidden="true"></i>
                            <a href="<?php echo esc_url( $cmt_link ); ?>"><?php echo esc_html( $log_book_comment );?></a>
                        </li>
                    <?php endif; ?>
			</ul>
		</div>
